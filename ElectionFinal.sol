...
contract Election{
    ...
    //return all the registed candidates
    ...
    function candiateLogin(string memory _name) view public returns(bool){
      
        for(uint i=0; i<candidates.length; i+=1){
            // Candidate storage candi = candidates[i];
    ...
    function adminLogin() ownerOnly public view returns(string memory){
       return "Success";
    }
