...
contract Deal {
 ...
 /// The Photo Verification struct
  ...
  function sendOrder(string goods, uint quantity, string photoURL, string videoURL) payable public {
    ...
    photos[orderseq] = Photo(orderseq, photoURL, videoURL, buyerAddr, owner, 0, Verification(false, false), Verification(false, false), true);
...
function SendPhotoVerification(uint orderno, int8 ptype) payable public {
    ...
    /// Trigger the event
    PhotoVerified(orderno, _photo.photoURL, (_photo.photoVerification.isVerifiedByCourier && _photo.photoVerification.isVerifiedByManufacturer), (_photo.videoVerification.isVerifiedByCourier && _photo.videoVerification.isVerifiedByManufacturer));
